# Experiment notes (historical snapshot)

The original CLAUDE.md is preserved verbatim below as of 2026-09-08.
Use AGENTS.md for current shared rules and HANDOFF_3060.md for the RTX 3060 RT-DETR assignment.
The snapshot describes the original two-model scope; the handoff describes a four-model comparison.
Its statement that this directory is not a Git repository is obsolete. Follow the current .gitignore,
which keeps subset manifests tracked. Environment versions, timing measurements, pipeline completion,
and the personal plan path below are historical records, not verification of this machine's current state.
Do not infer current training progress from these notes.
